...
var ndviRGB = dataset.select('NDVI').mean().clip(toronto).visualize(ndviVis);

var gard_ndviRGB = gard_dataset.select('NDVI').mean().clip(gardiner).visualize(ndviVis);

// Export as colored GeoTIFF
Export.image.toDrive({
  image: ndviRGB,
  description: 'toronto_ndvi_color_export_nd',
  crs: 'EPSG:4326',
...
Export.image.toDrive({
  image: gard_ndviRGB,
  description: 'gardiner_ndvi_color_export_nd',
  crs: 'EPSG:4326',
